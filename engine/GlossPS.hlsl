...
float4 psmain(PS_INPUT input) : SV_TARGET
{
	...
	//float3 lightFinal = rimLighting + diffuseReflection + (specularReflection * gloss.x) + m_ambient_light_color;
	...
	float3 gloss = GlossTexture.Sample(TextureSampler, input.texcoord);
	specular_reflection *= gloss;
