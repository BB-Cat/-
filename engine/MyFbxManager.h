...
public:
	...
	std::vector<MeshData> loadFbxMesh(const wchar_t* filename, float* topology);
	...
	void loadAnimationData(const wchar_t* filename, SkeletalAnimation& skeletal_animation, u_int sampling_rate = 0);
private:
	...
	void fetchAnimations(FbxMesh* fbx_mesh, SkeletalAnimation& skeletal_animation, u_int sampling_rate = 0);
